...
Each preset displays estimated cost per image based on:
- **Model:** Default or specified in preset
- **Resolution:** `default_resolution` in preset (512, 1K, 2K, 4K)

**ESTIMATED BY:** `skills/banana/scripts/presets.py`  
   See: `cmd_list()` and `cmd_show()` functions  
   Usage: `presets.py list` (shows cost column), `presets.py show NAME` (shows breakdown)  
   Config source: `skills/banana/config.json`  

Examples:
- tech-saas preset at 2K (2x multiplier): $0.0020/image ($0.20 per 100)
- Budget preset at 1K (1x multiplier): $0.0005/image ($0.05 per 100)
