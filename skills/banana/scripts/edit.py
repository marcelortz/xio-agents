...
Usage:
    edit.py --image path/to/image.png --prompt "remove the background"
            [--model MODEL] [--api-key KEY] [--aspect-ratio RATIO] [--resolution RES]
"""
...
def main():
    ...
    parser.add_argument("--aspect-ratio", default=DEFAULT_ASPECT_RATIO, help=f"Output aspect ratio (optional, default: {DEFAULT_ASPECT_RATIO})")
    parser.add_argument("--resolution", default=DEFAULT_RESOLUTION, help=f"Output resolution: 512, 1K, 2K, 4K (optional, default: {DEFAULT_RESOLUTION})")

    args = parser.parse_args()

    # Validate image configuration if aspect-ratio or resolution specified
    try:
        config = ImageConfig(
